# Civilization Odyssey 1.2.0

中文 | [English](#english)

Civilization Odyssey 为 WorldBox 增加一条缓慢、可观察的文明发展路线。城市会从中世纪逐步进入工坊、工业与现代阶段，国家会研究科技、建设公共设施、选择政策、处理危机，并在战争与发展之间分配真实库存。

## 1.2.0 更新

- 工坊、工业、现代的最低城龄改为8、20、40年；提高年度金属产出，缺金属时优先寻找可采金属矿。
- 自动军购和原版制造为工坊、工业、现代城市分别保留10、18、4金属，给发展和科研留出库存。
- 飞升者名单支持逐人查看、候选者画笔、按比例随机授予和一键授予所有国王。
- 城市归属窗口支持多选城市，强制划归现有国家或共同建国。
- 榴弹发射器和轮式野战炮有独立的地图外观；玩家核打击取消年份冷却，每次发射都会降低各国对发射国的好感。

游戏界面支持简体中文与 English，并跟随 WorldBox 的语言设置。详细机制见 [PLAYER_GUIDE.md](PLAYER_GUIDE.md)，安装步骤见 [INSTALL.md](INSTALL.md)。

运行本模组需要 NeoModLoader，并需在 WorldBox 中开启 Experimental Mode。首次使用 NML 时请先按其安装说明手动安装加载器。

## 兼容环境

- Windows
- WorldBox 0.51.2，Steam build 19962337
- NeoModLoader 1.2.0.1

其他系统、游戏版本和模组组合尚未确认兼容。

## 主要内容

- 工坊、工业、现代三个发展阶段，升级取决于人口、城龄、库存和科技。
- 工业、民生、军事三条研究路线，共11项科技。
- 按游戏年结算的工坊与工厂生产，以及农业、工业、学术城市分工。
- 公共粮仓、市立医院、城市学府与同国内部粮运。
- 国家政策、稳定、国内事件、战后重建和教育改革。
- 贸易、互不侵犯、禁运、援助、现代军备与后期核工程。
- 默认年度动员、可批量使用的军械援助，以及叛乱/征服后的城市多数科技继承。
- 可选的飞升候选者成长玩法，附名单和批量授予工具。
- 多城强制划归与联合建国，保留原有多城联合起义。

## 五分钟上手

1. 安装并启用模组后，创建新世界或载入世界副本。
2. 打开 **Civilization Odyssey → 总览 → 国家面板**，再点击一座城市。
3. 第一次游玩保留默认自动设置，先查看城市“为什么没有发展”。
4. 关闭面板并恢复时间。大多数系统按游戏年结算。
5. 需要干预时，再调整研究、政策、分工、建设、军备或外交。

飞升工具位于底部“成长”页；多选归属工具位于“神力 → 城市归属”。国家面板在功能名称附近说明用途，费用和完整规则可点“详情”查看。

自然工业化不是固定倒计时。粮食、战争、人口、库存和研究都会改变节奏，几十年后才出现工业城市很正常。

## English

Civilization Odyssey adds a slow, observable path of civilization development to WorldBox. Cities progress from the medieval stage into workshops, industry, and the modern era. Countries research technologies, build public services, choose policies, respond to crises, and divide real stock between war and development.

### 1.2.0 changes

- Workshop, Industrial, and Modern eras now require city ages of 8, 20, and 40 years. Yearly metal output is higher, and miners prioritize reachable metal deposits when city stock is low.
- Automatic procurement and vanilla crafting reserve 10, 18, and 4 metal in Workshop, Industrial, and Modern cities for development and research.
- The ascendant roster includes individual inspection, a candidate brush, random percentage grants, and a grant-all-kings action.
- Select several cities to transfer them to an existing country or found one country together.
- Grenade launchers and wheeled field artillery have their own world sprites. Player nuclear strikes have no year cooldown; each launch lowers other countries' opinion of the attacker.

The in-game interface supports Simplified Chinese and English and follows the WorldBox language setting. See [PLAYER_GUIDE.md](PLAYER_GUIDE.md) for rules and [INSTALL.md](INSTALL.md) for setup.

The mod requires NeoModLoader and WorldBox Experimental Mode. Follow the NML installation guide to install the loader manually before first use.

### Verified environment

- Windows
- WorldBox 0.51.2, Steam build 19962337
- NeoModLoader 1.2.0.1

Other operating systems, game versions, and mod combinations are not yet confirmed.

### Features

- Workshop, Industrial, and Modern eras gated by population, city age, stock, and technology.
- Eleven technologies across Industry, Civil, and Military branches.
- Yearly workshop and factory production, plus Agriculture, Industry, and Academic city roles.
- Public granaries, municipal hospitals, city academies, and domestic food logistics.
- National policy, stability, domestic events, postwar reconstruction, and education reform.
- Trade, non-aggression, embargoes, aid, modern armaments, and late-game atomic engineering.
- Default yearly mobilization, brush-based armament aid, and majority city-knowledge inheritance after rebellion or conquest.
- Optional Ascension Candidate progression with a roster and group grant tools.
- Forced city transfers and joint founding, alongside the existing Multi-City Uprising.

### Five-minute start

1. Install and enable the mod, then create a new world or load a copy of an existing one.
2. Open **Civilization Odyssey → Overview → National Panel**, then select a city.
3. Keep the default automatic settings for your first game and read why the city is stalled.
4. Close the panel and resume time. Most systems settle once per game year.
5. Adjust research, policy, city roles, construction, military supply, or diplomacy when intervention is useful.

Find ascendant tools under Growth in the bottom tab, and city selection under God tools → City ownership. National Panel pages explain each function beside its name; open Details for costs and full rules.

Natural industrialization is not a fixed countdown. Food, war, population, stock, and research all affect its pace, so taking decades to reach industry is normal.
